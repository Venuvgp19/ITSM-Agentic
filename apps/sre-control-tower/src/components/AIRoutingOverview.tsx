...
export const AIRoutingOverview: React.FC = () => {
  ...
  return (
    <div className="space-y-6 font-sans text-slate-100 animate-fadeIn">
      {/* Header Banner */}
      <div className="flex flex-col md:flex-row md:items-center justify-between bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h2 className="text-lg font-bold text-slate-100">Autonomous Agentic AI Router</h2>
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center space-x-3 bg-slate-950 px-4 py-2 rounded-xl border border-slate-800">
            ...
            <span className="text-sm font-bold text-cyan-400 font-mono">{threshold}%</span>
            <input
              type="range"
              min="60"
              max="95"
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-semibold text-slate-200 border border-slate-700 cursor-pointer transition-all"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl relative overflow-hidden">
          ...
          <p className="text-2xl font-black text-slate-100 font-mono">{incidents.length.toLocaleString()}</p>
          <span className="text-[10px] text-slate-400">100% Live DB Grounded</span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          ...
          <p className="text-2xl font-black text-emerald-400 font-mono">{autoAssignRate}%</p>
          <span className="text-[10px] text-slate-400">Confidence ≥ {threshold}% ({autoAssignedCount.toLocaleString()} tickets)</span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          ...
          <p className="text-2xl font-black text-sky-400 font-mono">15.0s</p>
          <span className="text-[10px] text-slate-400">Autonomous Daemon Worker Loop</span>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 p-4 rounded-xl">
          ...
          <p className="text-sm font-bold text-indigo-400 font-mono mt-1">Model Context Protocol</p>
          <span className="text-[10px] text-slate-400">ITSM MCP Stdio Server</span>
        </div>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 p-5 rounded-2xl shadow-xl">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider mb-3 flex items-center gap-2">
          <BarChart3 className="w-4 h-4 text-cyan-400" /> Operational Assignment Distribution ({parsedAudits.length} Records)
        </h3>
          ...
          {Object.entries(queueStats).map(([dept, count]) => {
            ...
            return (
              ...
              >
                ...
                <div className="text-lg font-black text-slate-100 font-mono">{count}</div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-xl">
        <div className="p-4 bg-slate-950 border-b border-slate-800 flex items-center justify-between">
          <h3 className="text-sm font-semibold text-slate-200 flex items-center gap-2">
            <Layers className="w-4 h-4 text-cyan-400" /> Live AI Classification & Triage Stream
          </h3>
          <div className="flex items-center gap-2">
            ...
            {lastRefreshed && (
              <span className="text-[10px] text-slate-500 font-mono">Synced {lastRefreshed}</span>
            )}
          </div>
        </div>
          ...
          {filteredAudits.length === 0 ? (
            ...
            filteredAudits.slice(0, 100).map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`p-4 transition-all hover:bg-slate-800/40 ${
                ...
                >
                  ...
                  >
                    <div className="space-y-1.5 flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="text-xs font-black text-cyan-400 font-mono">{item.number}</span>
                        <span
                        ...
                        <span className="text-[10px] text-slate-400 font-mono">Lead: {item.assignedTo}</span>
                      </div>

                      <p className="text-xs font-semibold text-slate-200 truncate">{item.title}</p>
                      <p className="text-[11px] text-slate-400 font-mono line-clamp-1">"{item.reasoning}"</p>
                    </div>

                    <div className="text-right space-y-1 flex-shrink-0">
                      <div
                        className={`flex items-center justify-end space-x-1.5 text-xs font-mono font-bold ${
                          isAuto ? 'text-emerald-400' : 'text-amber-400'
                        }`}
                      >
                        {isAuto ? (
                      ...
                      <span className="text-[10px] text-slate-500 block font-mono">{item.time}</span>
                      <button className="text-[10px] text-cyan-400 flex items-center gap-0.5 ml-auto">
                  ...
                  {isExpanded && (
                    <div className="mt-3 pt-3 border-t border-slate-800/80 space-y-2 text-xs bg-slate-950/60 p-3 rounded-xl">
                      <div className="flex items-center justify-between text-slate-400">
                        <span className="font-semibold text-cyan-400">Diagnostic Thinking Trace:</span>
                        <span className="font-mono text-[10px] text-slate-500">CI Target: {item.ci}</span>
                      </div>
                      <p className="text-slate-300 font-mono text-[11px] leading-relaxed whitespace-pre-wrap bg-slate-900/80 p-2.5 rounded-lg border border-slate-800">
                        {item.trace}
                      </p>
                      <div className="flex items-center justify-between text-[10px] text-slate-400 pt-1">
                        <span>
                          ...
                          <span
                            className={
                              isAuto ? 'text-emerald-400 font-bold' : 'text-amber-400 font-bold'
                            }
                          >
                            {isAuto ? 'AUTONOMOUSLY ROUTED' : 'REQUIRES OPERATOR REVIEW'}
                          </span>
                        </span>
                        <span className="font-mono text-slate-500">Incident SysId: {item.id}</span>
                      </div>
                    </div>
                  )}
                </div>
