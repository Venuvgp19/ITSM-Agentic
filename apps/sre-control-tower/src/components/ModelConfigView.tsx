...
export function ModelConfigView() {
  ...
  if (loading) {
    return (
      <div className="pro-card rounded-2xl p-16 text-center flex flex-col items-center justify-center shadow-xl">
        <RefreshCw className="w-8 h-8 animate-spin text-cyan-600 dark:text-cyan-400 mb-3" />
        <span className="text-xs font-bold text-slate-500 dark:text-slate-400">Loading Environment & Model Configuration...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto font-sans">
      {toastMessage && (
        <div className="bg-emerald-500/20 border border-emerald-500/40 text-emerald-700 dark:text-emerald-300 p-4 rounded-xl flex items-center justify-between shadow-lg shadow-emerald-950/20">
      ...
      {/* Header Banner */}
      <div className="pro-card rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-2xl relative overflow-hidden flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <div className="inline-flex items-center gap-2 bg-cyan-500/20 text-cyan-700 dark:text-cyan-300 text-xs font-bold px-3 py-1 rounded-full mb-2 border border-cyan-500/30">
            <Sliders className="w-3.5 h-3.5" />
            <span>Multi-Environment Model Governance Engine</span>
          </div>
          <h2 className="text-xl md:text-2xl font-black text-slate-900 dark:text-white tracking-tight">AI Model & Router Configuration</h2>
          <p className="text-slate-500 dark:text-slate-400 text-xs mt-1 max-w-2xl font-medium">
        ...
        <div className="flex flex-col items-start md:items-end shrink-0">
          <span className="text-[10px] text-slate-500 dark:text-slate-400 font-bold uppercase tracking-wider">Active Mode</span>
          <span className="mt-1 bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 text-xs font-extrabold px-3 py-1 rounded-lg border border-emerald-500/40 flex items-center gap-1.5 shadow-sm">
      ...
      <div className="pro-card rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <button
            ...
            className={`p-5 rounded-xl border text-left transition-all relative cursor-pointer ${
              config.environment === 'genai_lab'
                ? 'bg-cyan-50 dark:bg-cyan-950/40 border-cyan-500/60 text-slate-900 dark:text-white shadow-lg shadow-cyan-950/30'
                : 'bg-white/50 dark:bg-slate-900/50 border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300 hover:bg-slate-100/60 hover:dark:bg-slate-800/60 hover:border-slate-300 hover:dark:border-slate-700'
            }`}
          >
            {config.environment === 'genai_lab' && (
            ...
            <div className="mt-3 text-xs font-mono bg-white/80 dark:bg-slate-950/80 p-2 rounded-lg text-cyan-700 dark:text-cyan-300 border border-slate-200 dark:border-slate-800">
          ...
          >
            ...
            <div className="mt-3 text-xs font-mono bg-white/80 dark:bg-slate-950/80 p-2 rounded-lg text-emerald-700 dark:text-emerald-300 border border-slate-200 dark:border-slate-800">
      ...
      <div className="pro-card rounded-2xl p-6 border border-slate-200/80 dark:border-slate-800/80 shadow-xl space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">Base API URL Endpoint</label>
            <input
              type="text"
              value={config.baseUrl}
              onChange={(e) => setConfig({ ...config, baseUrl: e.target.value })}
              className="focus-ring w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-slate-100 font-mono focus:border-cyan-500 transition"
              placeholder="https://genailab.tcs.in/v1"
            />
          </div>

          <div>
            <label className="block text-xs font-bold text-slate-500 dark:text-slate-400 mb-1.5 uppercase tracking-wide">API Access Key Token</label>
            <div className="relative">
              <input
                type={showApiKey ? 'text' : 'password'}
                value={config.apiKey}
                onChange={(e) => setConfig({ ...config, apiKey: e.target.value })}
                className="focus-ring w-full bg-white dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-xs text-slate-900 dark:text-slate-100 font-mono focus:border-cyan-500 pr-10 transition"
                placeholder="sk-..."
              />
              <button
                type="button"
                ...
                className="focus-ring absolute right-3 top-2.5 text-slate-500 dark:text-slate-400 hover:text-slate-900 hover:dark:text-white cursor-pointer"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="p-4 bg-white/60 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-cyan-600 dark:text-cyan-400">Classification</span>
            </div>
            <select
              value={config.routerModel}
              onChange={(e) => setConfig({ ...config, routerModel: e.target.value })}
              className="focus-ring w-full bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-700 dark:text-slate-200 focus:border-cyan-500 font-mono"
            >
          ...
          <div className="p-4 bg-white/60 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-cyan-600 dark:text-cyan-400">SSH & Runbook</span>
            </div>
            <select
          ...
          <div className="p-4 bg-white/60 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-purple-600 dark:text-purple-400">RAG Deduplication</span>
            </div>
            <select
          ...
          <div className="p-4 bg-white/60 dark:bg-slate-950/60 rounded-xl border border-slate-200/80 dark:border-slate-800/80 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] font-mono text-emerald-600 dark:text-emerald-400">Safety & HITL</span>
            </div>
            <select
