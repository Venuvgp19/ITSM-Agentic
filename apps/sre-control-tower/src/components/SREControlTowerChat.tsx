...
export const SREControlTowerChat: React.FC = () => {
  ...
  const renderInlineFormatting = (text: string) => {
    ...
    return parts.map((part, i) => {
      ...
      if (part.startsWith('`') && part.endsWith('`')) {
        return (
          <code key={i} className="px-1.5 py-0.5 bg-slate-100 dark:bg-black/40 text-emerald-700 dark:text-emerald-400 rounded-md font-mono text-[11px] border border-slate-200 dark:border-white/10">
            {part.slice(1, -1)}
          </code>
        );
      }
    ...
    while (i < lines.length) {
      ...
      if (line.trim().startsWith('|') && line.includes('|') && i + 1 < lines.length && lines[i + 1].trim().startsWith('|') && lines[i + 1].includes('-')) {
        ...
        if (tableLines.length >= 2) {
          ...
          elements.push(
            <div key={`table-${i}`} className="my-2.5 overflow-x-auto rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-black/20 shadow-inner">
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="bg-slate-100 dark:bg-white/[0.04] border-b border-slate-200 dark:border-white/10 text-cyan-700 dark:text-cyan-300 font-semibold">
                ...
                <tbody className="divide-y divide-slate-200 dark:divide-white/[0.05]">
                  {rows.map((row, rIdx) => (
                    ...
                    >
                      {row.map((cell, cIdx) => (
                        <td key={cIdx} className="px-3 py-1.5 text-slate-700 dark:text-slate-200 font-mono text-[11px]">
      ...
      if (line.startsWith('### ')) {
        elements.push(<h4 key={i} className="font-semibold text-cyan-700 dark:text-cyan-300 text-[13px] mt-2.5 mb-1">{line.replace('### ', '')}</h4>);
        i++;
        continue;
      }
      if (line.startsWith('## ')) {
        elements.push(<h3 key={i} className="font-bold text-slate-900 dark:text-white text-sm mt-3 mb-1">{line.replace('## ', '')}</h3>);
        i++;
        continue;
      }
      if (line.startsWith('# ')) {
        elements.push(<h2 key={i} className="font-extrabold text-slate-900 dark:text-white text-base mt-3.5 mb-1.5">{line.replace('# ', '')}</h2>);
        i++;
        continue;
      }
      ...
      if (line.startsWith('- ') || line.startsWith('* ')) {
        ...
        elements.push(
          <li key={i} className="ml-4 list-disc marker:text-cyan-500/60 text-slate-700 dark:text-slate-200 text-[13px] leading-relaxed my-0.5">
      ...
      if (numberedMatch) {
        elements.push(
          <div key={i} className="flex items-start space-x-1.5 my-0.5 text-[13px] text-slate-700 dark:text-slate-200 ml-1">
            <span className="font-mono text-cyan-600 dark:text-cyan-400 font-bold">{numberedMatch[1]}.</span>
      ...
      if (line.startsWith('> ')) {
        elements.push(
          <div key={i} className="border-l-2 border-cyan-500/50 pl-2.5 py-0.5 my-1 text-slate-500 dark:text-slate-400 italic text-[13px]">
      ...
      elements.push(
        <p key={i} className="text-[13px] leading-relaxed my-0.5 text-slate-700 dark:text-slate-200">
    ...
    <>
      ...
      {!isOpen && (
        ...
        >
          <div className="relative">
            ...
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-emerald-400 rounded-full border-2 border-white dark:border-slate-950 animate-pulse" />
          </div>
      ...
      {isOpen && position && (
        <div
          ...
          className={`fixed z-50 flex flex-col bg-white/95 dark:bg-[rgba(10,13,22,0.92)] backdrop-blur-2xl border border-slate-200 dark:border-white/10 rounded-[20px] shadow-[0_25px_60px_-15px_rgba(15,23,42,0.25)] dark:shadow-[0_25px_60px_-15px_rgba(0,0,0,0.75)] overflow-hidden select-text transition-shadow duration-200 ${
        ...
        >
          ...
          <div
            ...
            className="relative bg-slate-50 dark:bg-black/20 px-4 py-3 border-b border-slate-200 dark:border-white/[0.07] flex items-center justify-between cursor-move select-none group"
            title="Click and drag to move window anywhere on screen"
          >
            <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-cyan-500/40 to-transparent" />
            <div className="flex items-center space-x-2.5">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-cyan-500/25 to-violet-600/25 border border-slate-200 dark:border-white/10 flex items-center justify-center shadow-inner">
                <Bot className="w-4 h-4 text-cyan-700 dark:text-cyan-300" />
              </div>
              <div>
                <div className="flex items-center space-x-2">
                  <span className="text-[13px] font-bold text-slate-900 dark:text-white tracking-wide">SRE Control Tower Assistant</span>
                  <span className="flex items-center gap-1 px-1.5 py-[3px] bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/30 text-[9px] font-semibold text-emerald-700 dark:text-emerald-300 rounded-full">
                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 dark:bg-emerald-400 animate-pulse" />
                    LIVE
                  </span>
                </div>
                <p className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center space-x-1 mt-0.5">
                  <Database className="w-2.5 h-2.5 text-cyan-600/70 dark:text-cyan-500/70" />
            ...
            <div className="flex items-center space-x-1">
              <div className="text-slate-400 dark:text-slate-600 group-hover:text-slate-500 px-1 transition-colors" title="Drag Window">
                <GripHorizontal className="w-4 h-4" />
              </div>
              <button
                onClick={clearChat}
                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-cyan-700 dark:hover:text-cyan-300 hover:bg-slate-200/70 dark:hover:bg-white/[0.06] rounded-lg transition-colors"
              ...
              <button
                ...
                className="p-1.5 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-200/70 dark:hover:bg-white/[0.06] rounded-lg transition-colors"
          ...
          {/* Quick Prompt Chips */}
          {messages.length <= 2 && (
            <div className="px-3.5 py-2.5 bg-slate-50 dark:bg-black/10 border-b border-slate-200 dark:border-white/[0.06] flex flex-wrap gap-1.5 overflow-x-auto select-none">
              {DEFAULT_PROMPTS.map((p, i) => (
                <button
                  key={i}
                  onClick={() => handleSend(p.prompt)}
                  className="px-2.5 py-1.5 bg-white dark:bg-white/[0.03] hover:bg-cyan-50 dark:hover:bg-cyan-500/[0.1] border border-slate-200 dark:border-white/[0.08] hover:border-cyan-400 dark:hover:border-cyan-500/40 text-[11px] text-slate-600 dark:text-slate-300 hover:text-cyan-700 dark:hover:text-cyan-200 rounded-full transition-all duration-150 flex items-center space-x-1.5"
                >
              ...
              >
                ...
                <div
                  className={`w-6 h-6 mt-0.5 rounded-full flex items-center justify-center flex-shrink-0 ${
                    msg.role === 'user'
                      ...
                      : 'bg-gradient-to-br from-cyan-500/25 to-violet-600/25 border border-slate-200 dark:border-white/10'
                  }`}
                >
                  {msg.role === 'user' ? (
                    <User className="w-3 h-3 text-slate-600 dark:text-slate-300" />
                  ) : (
                    <Bot className="w-3 h-3 text-cyan-700 dark:text-cyan-300" />
                  )}
                </div>
                ...
                <div className={`flex flex-col min-w-0 max-w-[86%] ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                  ...
                  <div className={`flex items-center gap-1.5 mb-1 px-1 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
                    ...
                    <span className="text-[9px] text-slate-400 dark:text-slate-600">{msg.timestamp}</span>
                  </div>

                  <div
                    className={`group/bubble relative rounded-2xl px-3.5 py-2.5 text-[13px] shadow-sm ${
                      msg.role === 'user'
                        ? 'bg-cyan-600 dark:bg-cyan-600/90 text-white rounded-tr-sm [&_p]:text-white [&_strong]:text-white [&_li]:text-white'
                        : 'bg-slate-50 dark:bg-white/[0.035] text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-white/[0.07] rounded-tl-sm'
                    }`}
                  >
                    {/* 12-Iteration Checkpoint Notification Banner */}
                    {msg.reachedMax && (
                      <div className="mb-2 p-2 bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/30 rounded-lg flex items-center space-x-2 text-[11px] text-amber-700 dark:text-amber-300">
                    ...
                    <div className="space-y-1">
                      {msg.content ? formatMarkdownContent(msg.content) : (
                        <span className="text-cyan-700 dark:text-cyan-300/90 text-[11px] flex items-center space-x-2">
                          <Search className="w-3 h-3 animate-pulse flex-shrink-0" />
                          <span className="flex gap-0.5">
                            <span className="w-1 h-1 rounded-full bg-cyan-600 dark:bg-cyan-400 animate-typing-dot" style={{ animationDelay: '0ms' }} />
                            <span className="w-1 h-1 rounded-full bg-cyan-600 dark:bg-cyan-400 animate-typing-dot" style={{ animationDelay: '150ms' }} />
                            <span className="w-1 h-1 rounded-full bg-cyan-600 dark:bg-cyan-400 animate-typing-dot" style={{ animationDelay: '300ms' }} />
                          </span>
                    ...
                    {msg.toolTraces && msg.toolTraces.length > 0 && (
                      <div className="mt-2.5 pt-2 border-t border-slate-200 dark:border-white/[0.08]">
                        <button
                          onClick={() => toggleTrace(msg.id)}
                          className="w-full flex items-center justify-between text-[10px] text-cyan-700 dark:text-cyan-300/90 hover:text-cyan-800 dark:hover:text-cyan-200 bg-white dark:bg-black/20 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-white/[0.06] transition-colors"
                        >
                        ...
                        {openTraces[msg.id] && (
                          <div className="mt-1.5 space-y-1.5 max-h-48 overflow-y-auto custom-scrollbar">
                            {msg.toolTraces.map((trace, tIdx) => (
                              <div key={tIdx} className="p-2 bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/[0.06] rounded-lg text-[10px] font-mono">
                                <div className="flex items-center justify-between text-slate-500 mb-1">
                                  <span className="text-cyan-700 dark:text-cyan-300/90 font-semibold">{trace.db}</span>
                                  <span>{trace.rowCount !== undefined ? `${trace.rowCount} rows` : ''}</span>
                                </div>
                                <div className="text-emerald-700 dark:text-emerald-400/90 break-all">{trace.query}</div>
                                {trace.error && <div className="text-rose-600 dark:text-rose-400 mt-1">Error: {trace.error}</div>}
                              </div>
                            ))}
                          </div>
                        )}
                    ...
                    {msg.role === 'assistant' && msg.content && (
                      <button
                        ...
                        className="absolute -bottom-2 -right-2 p-1 bg-white dark:bg-slate-800 border border-slate-200 dark:border-white/10 rounded-full text-slate-500 dark:text-slate-400 hover:text-cyan-700 dark:hover:text-cyan-300 opacity-0 group-hover/bubble:opacity-100 transition-opacity shadow-md"
                        title="Copy message"
                      >
                        {copiedId === msg.id ? <Check className="w-2.5 h-2.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="w-2.5 h-2.5" />}
                      </button>
                    )}
                  </div>
                </div>
          ...
          {/* Input Bar */}
          <div className="p-3 bg-slate-50 dark:bg-black/20 border-t border-slate-200 dark:border-white/[0.07] flex items-end space-x-2">
            <div className="flex-1 relative focus-within:shadow-[0_0_0_1px_rgba(6,182,212,0.4)] rounded-xl transition-shadow">
              <textarea
                ...
                rows={1}
                disabled={loading}
                className="focus-ring w-full bg-white dark:bg-white/[0.04] border border-slate-200 dark:border-white/10 rounded-xl px-3.5 py-2.5 text-[13px] text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:border-cyan-500/50 resize-none max-h-24 custom-scrollbar disabled:opacity-50 transition-colors"
              />
            </div>
            <button
              onClick={() => handleSend()}
              disabled={!input.trim() || loading}
              className="p-2.5 bg-gradient-to-br from-cyan-500 to-violet-600 hover:from-cyan-400 hover:to-violet-500 disabled:bg-slate-100 dark:disabled:bg-white/[0.05] disabled:bg-none disabled:text-slate-400 dark:disabled:text-slate-600 text-white rounded-xl transition-all shadow-sm flex items-center justify-center"
              title="Send Query"
            >
          ...
          <div
            ...
            className="absolute bottom-0 right-0 w-4 h-4 cursor-se-resize z-20 flex items-end justify-end p-0.5 text-slate-400 dark:text-slate-500 hover:text-cyan-600 dark:hover:text-cyan-400 transition"
