    ...
    return (
      <div className="p-8 space-y-4 animate-pulse">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-28 bg-slate-800/40 rounded-xl"></div>)}
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#111827] p-6 rounded-2xl border border-slate-800 shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2.5">
              <h2 className="text-xl font-black text-slate-100 tracking-tight">PROACTIVE PROBLEM ANALYSIS & IMPROVEMENT AGENT</h2>
              <span className="px-2.5 py-0.5 rounded-full text-[10px] font-mono bg-purple-500/10 text-purple-400 border border-purple-500/30 flex items-center gap-1 font-bold">
                <Sparkles className="w-3 h-3 text-purple-400" />
                CONTINUOUS IMPROVEMENT ENGINE
              </span>
            </div>
            <p className="text-xs text-slate-400 mt-0.5">AI-driven systemic root cause clustering, cost leakage prevention, MTBF modeling, and proactive problem promotion</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={refetch}
            className="px-4 py-2 bg-slate-900 hover:bg-slate-800 text-slate-200 border border-slate-700/80 rounded-xl text-xs font-bold flex items-center gap-2 transition active:scale-95"
          >
      ...
      {actionMessage && (
        <div className="p-4 bg-emerald-500/20 border border-emerald-500/50 rounded-xl text-emerald-300 text-xs font-bold flex items-center gap-2 shadow-lg">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3.5">
        <div className="bg-[#111827]/80 p-4 rounded-xl border border-slate-800/80 shadow-md">
          <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">Monthly Cost Leakage At Risk</div>
          <div className="text-2xl font-black text-emerald-400">${totalLeakage.toLocaleString()}</div>
        ...
        <div className="bg-[#111827]/80 p-4 rounded-xl border border-slate-800/80 shadow-md">
          <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">Systemic MTBF (Hours)</div>
          <div className="text-2xl font-black text-cyan-400">{avgMTBF} hrs</div>
        ...
        <div className="bg-[#111827]/80 p-4 rounded-xl border border-slate-800/80 shadow-md">
          <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">KEDB Knowledge Coverage</div>
          <div className="text-2xl font-black text-purple-400">{kedbCoverage}</div>
        ...
        <div className="bg-[#111827]/80 p-4 rounded-xl border border-slate-800/80 shadow-md">
          <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">Proactive Problem Detection</div>
          <div className="text-2xl font-black text-teal-400">{proactiveDetectionRate}</div>
        ...
        <div className="bg-[#111827]/80 p-4 rounded-xl border border-slate-800/80 shadow-md">
          <div className="text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">Proactive Problems Created</div>
          <div className="text-2xl font-black text-amber-400">{totalPromoted} / {totalCandidates}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-[#111827] p-5 rounded-2xl border border-slate-800 space-y-4 shadow-xl">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Identified from recurring incident clusters to prevent future SLA breaches</p>
          </div>
              ...
              >
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <span className={`px-2 py-0.5 text-[9px] font-bold rounded ${
                    candidate.priority.includes('P1') ? 'bg-red-500/20 text-red-400 border border-red-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/30'
                  }`}>
                    {candidate.priority}
                  </span>
                </div>

                <h4 className="text-xs font-bold text-slate-100 line-clamp-1">{candidate.title}</h4>
                <p className="text-[11px] text-slate-400 mt-1 line-clamp-2">{candidate.description}</p>

                <div className="mt-3 pt-2.5 border-t border-slate-800/80 flex items-center justify-between text-[11px] font-mono">
                  <span className="text-slate-400">CI: {candidate.configurationItem}</span>
                  <span className="text-emerald-400 font-bold">${candidate.costLeakage}/mo</span>
                  {candidate.promoted ? (
                    <span className="text-emerald-400 font-bold flex items-center gap-1">
                      <Check className="w-3 h-3" /> PROMOTED
                    </span>
                  ) : (
                    <span className="text-purple-400 font-bold">CANDIDATE</span>
                  )}
                </div>
              </div>
            ))}
        ...
        {activeCandidate && (
          <div className="lg:col-span-2 bg-[#111827] p-6 rounded-2xl border border-slate-800 space-y-6 shadow-xl">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800 pb-4 gap-3">
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="px-2.5 py-0.5 rounded text-[10px] font-mono bg-purple-500/20 text-purple-300 border border-purple-500/30 font-bold">
                    PROACTIVE CANDIDATE • {activeCandidate.number}
                  </span>
                  <span className="text-xs text-slate-400 font-mono">Target: {activeCandidate.configurationItem}</span>
                </div>
                <h3 className="text-lg font-black text-slate-100">{activeCandidate.title}</h3>
                <p className="text-xs text-slate-400 mt-0.5">{activeCandidate.description}</p>
              </div>
              ...
              {!activeCandidate.promoted ? (
                ...
                <div className="px-4 py-2 bg-emerald-500/20 border border-emerald-500/40 text-emerald-300 font-bold rounded-xl text-xs flex items-center gap-2 self-start sm:self-auto">
                  <CheckCircle2 className="w-4 h-4 text-emerald-400" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-1">
                ...
                <div className="text-xl font-black text-emerald-400">${activeCandidate.costLeakage.toLocaleString()} / mo</div>
                <div className="text-[11px] text-slate-400">Based on {activeCandidate.incidentCount} recurring tickets</div>
              </div>

              <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-1">
                <div className="text-[10px] font-mono text-slate-500 uppercase font-bold">Systemic MTBF</div>
                <div className="text-xl font-black text-cyan-400">{activeCandidate.mtbfHours} Hours</div>
                <div className="text-[11px] text-slate-400">Failure recurrence window</div>
              </div>

              <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-1">
                <div className="text-[10px] font-mono text-slate-500 uppercase font-bold">Predicted Outage Risk Score</div>
                <div className="text-xl font-black text-amber-400">{activeCandidate.riskScore} / 100</div>
                <div className="text-[11px] text-slate-400">Pre-SLA breach indicator</div>
              </div>
            </div>
            ...
            <div className="p-5 bg-purple-500/10 border border-purple-500/30 rounded-2xl space-y-2">
              ...
              <p className="text-xs text-purple-200 leading-relaxed">{activeCandidate.improvementRecommendation}</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-2">
                ...
                <div className="text-xs text-slate-200 font-medium">{activeCandidate.rootCause}</div>
              </div>

              <div className="p-4 bg-slate-950/60 rounded-xl border border-slate-800/80 space-y-2">
                <div className="text-[10px] font-mono text-slate-500 uppercase font-bold">Verified Engineering Workaround</div>
                <div className="text-xs text-cyan-300 font-mono">{activeCandidate.workaround}</div>
              </div>
            </div>
                ...
                {activeCandidate.fiveWhys.map((why, idx) => (
                  <div 
                    ...
                    className={`p-3 rounded-xl border text-xs transition ${
                      idx === 4 
                        ? 'bg-purple-500/10 border-purple-500/40 text-purple-200 font-semibold' 
      ...
      <div className="bg-[#111827] p-6 rounded-2xl border border-slate-800 space-y-5 shadow-xl">
        <div>
          ...
          <p className="text-xs text-slate-400">Proactive problem records identified to permanently eliminate recurring infrastructure defects</p>
        </div>
          ...
          <table className="w-full text-left text-xs text-slate-300">
            ...
            <tbody className="divide-y divide-slate-800/60 font-medium">
              {proactiveCandidates.map(c => (
                <tr key={c.number} className="hover:bg-slate-900/50 transition">
                  <td className="p-3.5">
                    <div className="font-mono text-purple-400 font-extrabold">{c.number}</div>
                  ...
                  <td className="p-3.5 text-slate-300 font-medium">{c.rootCause}</td>
                  <td className="p-3.5 font-mono text-emerald-400 font-bold">${c.costLeakage}/mo</td>
                  <td className="p-3.5">
                    {!c.promoted ? (
                      <button
                      ...
                      <span className="px-2.5 py-1 rounded bg-emerald-500/20 text-emerald-300 font-mono text-[11px] border border-emerald-500/30 flex items-center gap-1 w-fit">
